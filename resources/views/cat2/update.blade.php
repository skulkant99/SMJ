                ...
                <div class="page-body">
                    <div class="card tabs-card">
                        ...
                        <div class="card-header">
                            <h4>Update SUPERCATEGORY</h4>
                            <p class="text-muted m-b-10">Detail</p>
                            <ul class="breadcrumb-title b-t-default p-t-10">
                                <li class="breadcrumb-item"><a href="#!">SUPERCATEGORY</i></a></li>
                                <li class="breadcrumb-item"><a href="#!">Update SUPERCATEGORY</a></li>
                            </ul>
                        </div>
                        ...
                        <div class="card-block" id="reload_add">
                            <form action="{{url('backoffice/cat2/update')}}" method="post"
                                enctype="multipart/form-data">
                                {{ csrf_field() }}

                                <input type="hidden" name="id" value="{{$data->categorysub_2_id}}">
                                ...
                                <select class="form-control" name="type" id="category">
                                    ...
                                    <option value="{{$key->category_id}}"
                                        {{$data->category_id == $key->category_id ? 'selected' : ''}}>
                                        {{$key->category_name_th}} / {{$key->category_name_en}}</option>
                                    @endforeach
                                    @endif
                                </select><br>
                                ...
                                <select class="form-control" name="subcategory" id="subcategory">
                                    ...
                                    <option value="{{$type->categorysub_id}}"
                                        {{$data->categorysub_id == $type->categorysub_id ? 'selected' : ''}}>
                                        {{$type->categorysub_name_th}} / {{$type->categorysub_name_en}}</option>
                                    @endforeach
                                    @endif
                                </select><br>
                                <label>SUPERCATEGORY (ภาษาไทย) :</label><span style="color:red"></span>
                                <input type="text" class="form-control" name="name_th"
                                    value="{{$data->categorysub_2_name_th}}"><br>

                                <label>SUPERCATEGORY (English) :</label><span style="color:red"></span>
                                <input type="text" class="form-control" name="name_en"
                                    value="{{$data->categorysub_2_name_en}}"><br>
